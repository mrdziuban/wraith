...
class Wraith::SaveImages
  ...
  def construct_command(width, url, file_name, selector, before_start, global_before_capture, path_before_capture)
    ...
    command_to_run = "#{meta.engine} #{wraith.phantomjs_options} '#{wraith.snap_file}' '#{url}' '#{width}' '#{file_name}' '#{selector}' '#{before_start}' '#{global_before_capture}' '#{path_before_capture}'"
