...
class Wraith::SaveImages
  ...
  def define_individual_job(label, settings, width)
    ...
    jobs = []
    jobs << [label, settings.path, prepare_widths_for_cli(width), settings.base_url,    base_file_name,    settings.selector, wraith.before_start, wraith.before_capture, settings.before_capture, wraith.resources_to_ignore]
    jobs << [label, settings.path, prepare_widths_for_cli(width), settings.compare_url, compare_file_name, settings.selector, wraith.before_start, wraith.before_capture, settings.before_capture, wraith.resources_to_ignore] unless settings.compare_url.nil?
  ...
  def parallel_task(jobs)
    Parallel.each(jobs, :in_threads => 8) do |_label, _path, width, url, filename, selector, before_start, global_before_capture, path_before_capture, resources_to_ignore|
      begin
        command = construct_command(width, url, filename, selector, before_start, global_before_capture, path_before_capture, resources_to_ignore)
  ...
  def construct_command(width, url, file_name, selector, before_start, global_before_capture, path_before_capture, resources_to_ignore)
    ...
    command_to_run = "#{wraith.binary_path} #{wraith.phantomjs_options} '#{wraith.snap_file}' '#{url}' '#{width}' '#{file_name}' '#{selector}' '#{before_start}' '#{global_before_capture}' '#{path_before_capture}' '#{resources_to_ignore.join(',')}'"
